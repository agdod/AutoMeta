...
$fileNameList = array('null');
  ...
  function openReadDir ($dirPath) { //open and read contents of gallery directory
    global $fileNameList;  
    $isImageXml = "null"; // initalise $isImageXml to null in case first file is non image
    if (is_dir($dirPath)){ //open directory and read contents
        if ($dh = opendir($dirPath)){
            $i=0;
            while (($file = readdir($dh)) !== false){
              $imageFileType = strtolower(pathinfo($file,PATHINFO_EXTENSION));  //check file extension isxml
